...
class PaidComplicationProvider: ComplicationProviderService() {
    ...
    override fun onComplicationActivated(complicationId: Int, type: Int, complicationManager: ComplicationManager) {
        ...
        getSimpleData()
        val paid = PreferenceManager.getDefaultSharedPreferences(this).getString("paid", "")
        val complicationData = ComplicationData.Builder(ComplicationData.TYPE_LONG_TEXT)
                .setShortText(ComplicationText.plainText(paid))
                .build()
    ...
    override fun onComplicationUpdate(complicationId: Int, dataType: Int,
        ...
        val paid = PreferenceManager.getDefaultSharedPreferences(this).getString("paid", "")
        val complicationData = ComplicationData.Builder(ComplicationData.TYPE_LONG_TEXT)
                .setLongText(ComplicationText.plainText(paid))
                .build()
        complicationManager.updateComplicationData(complicationId, complicationData)
    }
    ...
    private fun getSimpleData(){
        CoroutineScope(Dispatchers.IO + CoroutineExceptionHandler { _, _ ->
            ...
            val nodeListTask = Wearable.getNodeClient(this@PaidComplicationProvider).connectedNodes
            val nodes = Tasks.await(nodeListTask)
            val sendMessage = Wearable.getMessageClient(this@PaidComplicationProvider).sendMessage(nodes[0].id,
                "simpleData", "Paid".toByteArray())
