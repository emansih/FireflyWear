...
class BillsToPayComplicationProvider: ComplicationProviderService() {
    ...
    override fun onComplicationActivated(complicationId: Int, type: Int, complicationManager: ComplicationManager) {
        ...
        getSimpleData()
        val billsToPay = PreferenceManager.getDefaultSharedPreferences(this).getString("billsToPay", "")
        val complicationData = ComplicationData.Builder(ComplicationData.TYPE_LONG_TEXT)
                .setLongText(ComplicationText.plainText(billsToPay))
                .build()
    ...
    private fun getSimpleData(){
        CoroutineScope(Dispatchers.IO + CoroutineExceptionHandler { _, _ ->
            ...
            val nodeListTask = Wearable.getNodeClient(this@BillsToPayComplicationProvider).connectedNodes
            val nodes = Tasks.await(nodeListTask)
            val sendMessage = Wearable.getMessageClient(this@BillsToPayComplicationProvider).sendMessage(nodes[0].id,
                "simpleData", "Bills To Pay".toByteArray())
