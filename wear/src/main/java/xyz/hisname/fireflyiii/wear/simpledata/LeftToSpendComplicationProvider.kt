...
class LeftToSpendComplicationProvider: ComplicationProviderService()  {
    ...
    override fun onComplicationActivated(complicationId: Int, type: Int, complicationManager: ComplicationManager) {
        ...
        getSimpleData()
        val leftToSpend = PreferenceManager.getDefaultSharedPreferences(this).getString("leftToSpend", "")
        val complicationData = ComplicationData.Builder(ComplicationData.TYPE_LONG_TEXT)
                .setLongText(ComplicationText.plainText(leftToSpend))
                .build()
    ...
    private fun getSimpleData(){
        CoroutineScope(Dispatchers.IO + CoroutineExceptionHandler { _, _ ->
            ...
            val nodeListTask = Wearable.getNodeClient(this@LeftToSpendComplicationProvider).connectedNodes
            val nodes = Tasks.await(nodeListTask)
            val sendMessage = Wearable.getMessageClient(this@LeftToSpendComplicationProvider).sendMessage(nodes[0].id,
                "simpleData", "Left To Spend".toByteArray())
